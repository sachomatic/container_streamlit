...
def change_password_dialog():
    ...
    if st.button("Changer le mot de passe"):
        db.update(
            (f"name {st.session_state['user'].name}",), (f"password {new_password}",)
        )
...
with db.db:
    ...
    def find_admin():
        ...
        with lock:
            users = db.read(
                []
            )  # Récupère toutes les lignes (dans ce cas tous les utilisateurs)
        users_with_admin = []
        for user in users:
            permissions = json.loads(
                user[2]
            )  # Les permissions sont stockées à la troisième position
            if (
                permissions[-1]["value"] is True
            ):  # Si la dernière permission (admin) est True
    ...
    def find_user(name, password):
        ...
        for user in users:
            if (
                user[1] == password
            ):  # Si le mot de passe correspond, on retourne le statut de la recherche et l'utilisateur
                return (True, user)
    ...
    def pass_admin(user):
        ...
        perms_dict = [
            {"name": perm.name, "value": perm.value} for perm in permissions_objs
        ]
        with lock:
            db.update(
                (f"name {user.name}",), (f"permissions {json.dumps(perms_dict)}",)
            )  # On mets les permissions à jour
        st.balloons()
        st.toast(
            "Bravo, vous êtes maintenant administrateur. Mise à jour...", icon="✅"
        )  # On affiche un toast
        st.session_state["user"].permissions[
            -1
        ].value = True  # On change les variables de session
        st.rerun()
        ...
        with st.form(key="login"):
            ...
            if st.form_submit_button("Se connecter"):
                ...
                if search[0]:  # Si oui, alors c'est un succès
                    ...
                    time.sleep(
                        2
                    )  # On attends 2 secondes avant de rediriger (pour les ballons)
                    st.rerun()
                elif (
                    search[1] == "Nom d'utilisateur incorrect"
                ):  # Si le nom d'utilisateur n'existe pas, on le communique à l'utilisateur
                    st.error("Nom d'utilisateur incorrect")
                elif (
                    search[1] == "Mot de passe incorrect"
                ):  # De même pour un mot de passe incorrect
    ...
    def register():
        ...
        with st.form(key="sign in"):
            ...
            if st.form_submit_button("S'inscrire"):
                ...
                )  # On crée un utilisateur avec les permissions de base
...
def login_page():
    ...
    if st.session_state["user"] is None:
        have_an_account = st.segmented_control(
            "Avez-vous un compte ?", ["Oui", "Non"], key="have_an_account"
        )
