...
def ctn_page():
    ...
    user = st.session_state["user"] if st.session_state["user"] is None else BASE_USER
    ...
    for container in containers:
        with st.container():
            ...
            start_help = (
                None if user.has_perm(1) else "Vous n'avez pas les permissions requises"
            )
