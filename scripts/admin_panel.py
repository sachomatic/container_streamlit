    ...
    with db.db:  # On utilise le Delta Generator pour fermer automatiquement la base de données à la fin de son utilisation
        ...
        if user is not None:  # Si l'utilisateur est connecté
            ...
            if user.has_perm(-1):
                ...
                users = db.read()
                for user in users:
                    name_col, perm_col, button_col = st.columns(
                        [0.3, 0.65, 0.05]
                    )  # On peut décider de la taille des colonnes
                    name_col.metric("Nom d'utilisateur", user[0])
                    show_password = name_col.segmented_control(
                        "Afficher le mdp",
                        ["Oui", "Non"],
                        key="show_password_" + user[0],
                        default="Non",
                    )
                    if show_password == "Oui":
                        name_col.write(user[1])
                    dte = perm_col.data_editor(
                        json.loads(user[2]), key="save_" + user[0]
                    )
                    button_col.button(
                        "💾",
                        help="Sauvegarder",
                        ...
                        key="save_" + str(user),
                    )  # Bouton de sauvegarde, appelant save_perms
        ...
        if len(find_admin()) == 0:
            ...
            no_admin_col1.info(
                "Aucun administrateur n'a encore ete crée. Voulez vous définir un administrateur?"
            )
